...
class Maga(asyncio.DatagramProtocol):
    ...
    async def handle_query(self, msg, addr):
        ...
        if query_type == b'get_peers':
            ...
            await self.handler({'q_type':'get_peers'
                                ,'info_hash':infohash})
            ...
            ip = bytes2string(addr[0])
            
            try:
                name = bytes2string(args[b'name'])
            except:
                name =''
            
            if b'implied_port' in args and args[b'implied_port']!=b'0':
                port = bytes2string(addr[1])
            else:
                port = bytes2string(args[b'port'])
                
            await self.handler({'q_type':'announce_peer'
                                ,'info_hash':infohash
                                ,'name':name
                                ,'ip':ip
                                ,'port':port})
